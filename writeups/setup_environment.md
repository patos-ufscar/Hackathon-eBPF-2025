...
```
7. Compile and execute runner.c:
```
```
8. Build and run main.go (userspace code):
```
GOOS=linux GOARCH=amd64 go build -o scheduler-cli
sudo KUBECONFIG=/etc/rancher/k3s/k3s.yaml ./scheduler-cli
```
NOTE: kubeconfig file changes depending on environment, this one is for k3s. When we did tests with minikube, it would be `KUBECONFIG=/etc/kubernetes/admin.conf`